...
Panel {
  ...
  // Remember the user's system default agent so the bar icon matches what
  // `omarchy agent` would launch, instead of defaulting to the alphabetically
  // first provider.
  ...
  FileView {
    ...
    printErrors: false
    onLoaded: {
      root.defaultAgentId = String(text() || "").trim()
      root.applyDefaultProvider()
    }
  }

  onProvidersChanged: applyDefaultProvider()

  function applyDefaultProvider() {
    if (root.defaultAgentId === "" || root.selectedProviderId !== "") return
    for (var i = 0; i < providers.length; i++) {
      if (providers[i].providerId === root.defaultAgentId) {
        root.selectedProviderId = root.defaultAgentId
        return
      }
    }
  }
